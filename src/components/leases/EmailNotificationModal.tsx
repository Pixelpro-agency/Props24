...
import type { LeaseRecipient } from '../../hooks/useLeaseRecipients';
...
export function EmailNotificationModal({ isOpen, onClose, recipients }: EmailNotificationModalProps) {
    ...
    // Update selected recipients when the default recipients list changes
    // to ensure the modal state stays in sync with table selection
    // Note: in a real implementation we might use a useEffect if the modal
    // remains mounted. Since AnimatePresence unmounts it, standard init works.
    ...
    function handleSend() {
        ...
        if (selected.length === 0 && sendMethod === 'email') {
            alert('Seleziona almeno un destinatario.');
            return;
        }

        console.log('Sending lease notification:', {
            method: sendMethod,
            subject,
            body,
            recipients: selected,
        ...
        // Trigger success feedback or API call ...
        handleClose();
    }
                        ...
                        <div className="bg-white rounded-lg shadow-xl w-full max-w-lg overflow-hidden" role="dialog">
                            ...
                            <div className="px-6 py-4 max-h-[500px] overflow-y-auto space-y-4">
                                ...
                                <div className="flex items-center gap-4">
                                    <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                        <input
                                            ...
                                            className="w-4 h-4 text-[#72a333] border-gray-300 focus:ring-[#72a333] accent-[#72a333]"
                                        />
                                        Invia per email
                                    </label>
                                    <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                                ...
                                {sendMethod === 'email' && (
                                    <div>
                                        ...
                                        <div className="space-y-1.5 max-h-[120px] overflow-y-auto border border-gray-200 rounded-md p-2">
                                            {recipients.map((r, index) => (
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        className="w-4 h-4 rounded border-gray-300 text-[#72a333] focus:ring-[#72a333] accent-[#72a333]"
                                                    />
                                            ...
                                            {recipients.length === 0 && (
                                                <p className="text-sm text-gray-400 italic p-1">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-3 py-2 border border-gray-200 rounded-md text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#72a333]/30 focus:border-[#72a333]"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-3 py-2 border border-gray-200 rounded-md text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#72a333]/30 focus:border-[#72a333] resize-y"
                                    />
                                </div>
                                ...
                                <div className="flex flex-wrap gap-4">
                                    <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors -ml-1">
                                        <input
                                            type="checkbox"
                                            checked={receiveCopy}
                                            onChange={(e) => setReceiveCopy(e.target.checked)}
                                            disabled={recipients.length === 0}
                                            className="w-4 h-4 rounded border-gray-300 text-[#72a333] focus:ring-[#72a333] accent-[#72a333] disabled:opacity-50"
                                        />
                            ...
                            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50">
                                <button
                                    ...
                                    className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-800 transition-colors cursor-pointer disabled:opacity-50"
                                    disabled={!body}
                                >
                                    {showPreview ? (
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <Button
                                        ...
                                        icon={Send}
                                        onClick={handleSend}
                                        disabled={sendMethod === 'email' && recipients.length === 0}
                                    >
                                        Invia
                                    </Button>
                                </div>
