...
const IDS = ['cadastre-file-test', 'key-test', 'contract-test', 'file-test'];
...
const payload = () => ({
    ...
    PropertyCadastreDocument: storedFile(IDS[0]),
    PropertyKeys: [{ id: IDS[1], description: 'Portone', number: '12', quantity: 1, holder: '', comments: '' }],
    PropertyContracts: [{ id: IDS[2], type: 'altro', description: 'Contratto', releaseDate: '', expiryDate: '', comments: '', file: storedFile(IDS[3]) }],
});
const idsOf = (value: ReturnType<typeof payload>) => [
    value.PropertyCadastreDocument?.id,
    value.PropertyKeys[0]?.id,
    value.PropertyContracts[0]?.id,
    value.PropertyContracts[0]?.file?.id,
];
...
describe('round-trip degli ID annidati Unit', () => {
    it('normalizzazione e draft sono idempotenti e preservano gli ID legacy byte-for-byte', () => {
        const input = payload();
        ...
        legacy.PropertyCadastreDocument!.id = 'cadastre-old-123';
        legacy.PropertyKeys[0].id = 'key-old-123';
        legacy.PropertyContracts[0].id = 'contract-old-123';
        legacy.PropertyContracts[0].file!.id = 'file-old-123';
        expect(idsOf(normalizePropertyFormData(legacy) as ReturnType<typeof payload>)).toEqual(['cadastre-old-123', 'key-old-123', 'contract-old-123', 'file-old-123']);
    });
